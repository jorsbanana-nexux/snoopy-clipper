...
| **Otak multimodal** | Gemini menerima transkrip **+ cuplikan frame** → menilai momen dari isi DAN kualitas visual (ekspresi, reaksi, aksi). |
| **Semua platform** | Apapun yang didukung yt-dlp: YouTube (termasuk Kids), TikTok, Instagram, X, dll. |
| **Momen berdaging** | Prompt ketat: konteks → membangun → pay-off. Basa-basi/iklan/momen asal ditolak. |
...
**⑥ Otak (Gemini)** — `brain.py` mengirim: (a) transkrip berformat `[12.4-15.6] teks per baris`, (b) semua frame jpg dalam urutan waktu + daftar timestamp-nya. Prompt memaksa: momen harus **utuh & berdaging** (konteks → membangun → pay-off), batas start/end harus timestamp transkrip asli, 15–90 dtk, anti tumpang tindih, diberi judul + hook + score. Jawaban JSON divalidasi: snap ke batas kata terdekat, clamp durasi, buang yang tumpang tindih, sort by score, potong ke `MAX_CLIPS`.
